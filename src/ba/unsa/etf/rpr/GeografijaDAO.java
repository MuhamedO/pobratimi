...
public class GeografijaDAO {
    ...
    private PreparedStatement glavniGradUpit, dajDrzavuUpit, obrisiDrzavuUpit, obrisiGradoveZaDrzavuUpit, nadjiDrzavuUpit,
            ...
            nadjiGradUpit, obrisiGradUpit, dajDrzaveUpit, dodajPobratimiZaGradUpit, obrisiPobratimiZaGradUpit, dajPobratimiZaGradUpit;
    ...
    private GeografijaDAO() {
        ...
        try {
            ...
            dodajPobratimiZaGradUpit = conn.prepareStatement("INSERT INTO pobratimi VALUES (?,?)");
            obrisiPobratimiZaGradUpit = conn.prepareStatement("DELETE FROM pobratimi where g1=? OR g2=?");
            dajPobratimiZaGradUpit = conn.prepareStatement("SELECT grad.id, grad.naziv, grad.broj_stanovnika, grad.drzava FROM grad, pobratimi WHERE pobratimi.g2=? AND pobratimi.g1=grad.id UNION " +
                    "SELECT grad.id, grad.naziv, grad.broj_stanovnika, grad.drzava FROM grad, pobratimi WHERE pobratimi.g1=? AND pobratimi.g2=grad.id");
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public void vratiBazuNaDefault() throws SQLException {
        ...
        stmt.executeUpdate("DELETE FROM pobratimi");
    ...
    private Grad dajGradIzResultSeta(ResultSet rs, Drzava d) throws SQLException {
        ...
        g.setPobratimi(dajPobratimiZaGrad(g));
        return g;
    }

    private ArrayList<Grad> dajPobratimiZaGrad(Grad g){
        ArrayList<Grad> rezultat = new ArrayList<>();
        try{
            dajPobratimiZaGradUpit.setInt(1, g.getId());
            dajPobratimiZaGradUpit.setInt(2, g.getId());
            ResultSet rs = dajPobratimiZaGradUpit.executeQuery();
            while(rs.next()){
                Grad g2 = new Grad(rs.getInt(1), rs.getString(2), rs.getInt(3), null);
                rezultat.add(g2);
            }
        } catch(SQLException e){
            e.printStackTrace();
    ...
    private void dodajPobratimiZaGrad(Grad grad) {
        try{
            for(Grad g : grad.getPobratimi()){
                dodajPobratimiZaGradUpit.setInt(1, grad.getId());
                dodajPobratimiZaGradUpit.setInt(2, g.getId());
                dodajPobratimiZaGradUpit.executeUpdate();
            }
        } catch(SQLException e){
            e.printStackTrace();
        }
    ...
    private void obrisiPobratimiZaGrad(Grad grad) {
        try {
            obrisiPobratimiZaGradUpit.setInt(1, grad.getId());
            obrisiPobratimiZaGradUpit.setInt(2, grad.getId());
            obrisiPobratimiZaGradUpit.executeUpdate();
        } catch(SQLException e){
            e.printStackTrace();
        }
    }
